...
public class CustomerServiceUtil {
    ...
    public static String constructQueryForFilter(RequestObjectForFilter requestObjectForFilter, int page, int size) {
        String query = "SELECT customer.id AS customer_id, building.name AS building_name, flat.room_number AS room_number, customer.phone, \n"
                + "\tcustomer.fullname, customer.citizen_id, customer.status AS customer_status, building_id\n"
                + queryGet;



        String search = requestObjectForFilter.getSearchString();
        query +=  "  AND (fullname LIKE \"%" + search
                + "%\" OR phone LIKE \"%" + search + "%\")\n";

        String buildingId = requestObjectForFilter.getBuildingId();
        if (buildingId != null && !buildingId.isBlank()) {
            query += " AND building.id = '" + buildingId + "'";
        }
        query += " ORDER BY building.name ASC, flat.room_number ASC, customer.status ASC "
                + " LIMIT " + size + " OFFSET " + page * size;
        return query;
    }
